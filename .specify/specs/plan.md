...
**Constraints**: No `unsafe` code; no external dependencies for core logic; all public items documented; MSRV to be pinned post-stabilization
**Scale/Scope**: Single crate, ~5 public API items (`LogicalPathContext`, `detect()`, `to_logical()`, `to_canonical()`, `has_mapping()`)
...
| **V. Semantic Versioning & MSRV** | ✅ PASS | Starting at `0.1.0`. MSRV to be pinned once implementation stabilizes and dependency requirements are known (per constitution TODO). |
...
| **V. Semantic Versioning & MSRV** | ✅ PASS | Version 0.1.0 confirmed. No external dependencies for core (Research R-007). MSRV deferred per constitution TODO. |
